...
preproc_acc <- function(raw_acc, sub, verbose = FALSE) {
  ...
  dat_acc <- raw_acc %>%
    ...
    mutate(
      ...
      # (e.g. -600).
      sessionTimestampLocal = as_datetime(session_timestamp) + hours(timezone / 100),
      sessionNumber = cumsum(sampleNumber == 1)
    ) %>%
    select(!c(firstKP_timestamp, session_timestamp, sampleNumber, diagnostic, 
              age, gender))
...
preproc_kp <- function(raw_kp, dat_acc, sub, verbose = FALSE) {
  ...
  dat_kp <- raw_kp %>%
    ...
    mutate(
      sessionNumber = cumsum(sampleNumber == 1),
      sessionTimestampLocal = as_datetime(session_timestamp) + hours(timezone / 100),
      keypressTimestampLocal = as_datetime(keypress_timestamp) + hours(timezone / 100),
    ...
    select(!c(firstKP_timestamp, keypress_timestamp, session_timestamp, 
              sampleNumber, diagnostic, age, gender))
